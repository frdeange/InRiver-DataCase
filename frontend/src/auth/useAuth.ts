...
// Safe to export one or the other since isDemoMode is a build-time constant.
// When demo mode is active, useMsalAuth is never invoked so no MsalProvider is needed.
export const useAuth: () => AuthState = isDemoMode ? useDemoAuth : useMsalAuth

// Expose demo users list for the login page selector
export { DEMO_USERS, DEMO_USER_KEY }

// Re-export msalInstance so App.tsx can pass it to MsalProvider without a circular import
export { msalInstance }
